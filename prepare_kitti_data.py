...
def split_by_video(training_dir, mapping, split_dir,
    ...
    new_calibs_dir = os.path.join(split_dir, 'calibs')
    if os.path.isdir(new_images_dir):
        shutil.rmtree(new_images_dir)
    if os.path.isdir(new_labels_dir):
        shutil.rmtree(new_labels_dir)
    if os.path.isdir(new_calibs_dir):
        shutil.rmtree(new_calibs_dir)

    for old_image_fname in os.listdir(os.path.join(training_dir, 'data_object_image_2', 'training', 'image_2')):
        old_image_path = os.path.abspath(os.path.join(training_dir, 'data_object_image_2', 'training', 'image_2', old_image_fname))
        ...
        old_label_path = os.path.abspath(os.path.join(training_dir, 'data_object_mod_label_2', 'training', 'label_2', old_label_fname))
        ...
        # Copy calibration
        old_calib_fname = '%s.txt' % image_index_str
        old_calib_path = os.path.abspath(os.path.join(training_dir, 'data_object_calib', 'training', 'calib', old_calib_fname))
        new_calib_fname = '%s_%s.txt' % (frame_id, image_index_str)
        new_calib_dir = os.path.join(new_calibs_dir, video_name)
        if not os.path.isdir(new_calib_dir):
            os.makedirs(new_calib_dir)
        new_calib_path = os.path.join(new_calib_dir, new_calib_fname)
        if use_symlinks:
            os.symlink(old_calib_path, new_calib_path)
        else:
            shutil.copyfile(old_calib_path, new_calib_path)
...
def split_for_training(split_dir, train_dir, val_dir,
    ...
    for images_dirname in os.listdir(os.path.join(split_dir, 'images')):
        ...
        if month == 9 and date == 26 and video_id <= 15:
            output_dir = val_dir
        else:
        ...
        # Copy calibrations
        old_calibs_dir = os.path.join(split_dir, 'calibs', images_dirname)
        new_calibs_dir = os.path.join(output_dir, 'calibs')
        if not os.path.isdir(new_calibs_dir):
            os.makedirs(new_calibs_dir)
        for fname in os.listdir(old_calibs_dir):
            old_calib_path = os.path.realpath(os.path.join(old_calibs_dir, fname))
            new_calib_path = os.path.join(new_calibs_dir, os.path.basename(old_calib_path))
            if use_symlinks:
                os.symlink(old_calib_path, new_calib_path)
            else:
                shutil.move(old_calib_path, new_calib_path)
...
if __name__ == '__main__':
    ...
    mapping = get_image_to_video_mapping(
        os.path.join(args.output_dir, 'raw', 'devkit_object'),
    )
