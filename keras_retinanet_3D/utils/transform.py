...
def transform_aabb(transform, aabb):
    ...
    Args
        ...
        xt:        
        yt:        
        orientation: 3D orientation ID
    Returns
        The new AABB as tuple (x1, y1, x2, y2, xl, yl, xm, ym, xr, yr, xt, yt, orientation)
    """
    x1, y1, x2, y2, xl, yl, xm, ym, xr, yr, xt, yt, orientation = aabb
    # Transform all points of the AABB.
    points = transform.dot([
        [x1, x2, x1, x2, xl, xm, xr, xt],
        [y1, y2, y2, y1, yl, ym, yr, yt],
        [1,  1,  1,  1,  1,  1,  1,  1 ],
    ])

    # Extract the min and max corners again.
    min_corner = points[:, :4].min(axis=1)
    max_corner = points[:, :4].max(axis=1)

    # Change 3D orientation ID in case of x flip
    if transform[0, 0] < 0:
        if orientation == 0:
            orientation = 1
        elif orientation == 1:
            ...
            orientation = 2
        return [min_corner[0], min_corner[1], max_corner[0], max_corner[1], points[0, 6], points[1, 6],
            points[0, 5], points[1, 5], points[0, 4], points[1, 4], points[0, 7], points[1, 7], orientation]

    return [min_corner[0], min_corner[1], max_corner[0], max_corner[1], points[0, 4], points[1, 4],
            points[0, 5], points[1, 5], points[0, 6], points[1, 6], points[0, 7], points[1, 7], orientation]

def transform_aabb_old(transform, aabb):
    """ Apply a transformation to an axis aligned bounding box.
    The result is a new AABB in the same coordinate system as the original AABB.
    The new AABB contains all corner points of the original AABB after applying the given transformation.
    Args
